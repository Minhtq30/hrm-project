<template>
    ...
    <template v-slot:sub_navbar v-if="isLoading == false">
      <nav-feature>
        <div class="flex justify-between items-center w-full">
          <div></div>
          <div class="flex items-center">
            <div class="btn-icon" @click="handleToEditGroup()">
              <div class="cx">
                <i class="fal fa-edit"></i>
              </div>
              <div class="action-label">Sửa</div>
            </div>
            <div class="btn-icon" @click="handleOpenConfirm()">
              <div class="cx">
        ...
        <div title="Thông tin chung" class="outer">
          ...
          <div class="w-full px-2">
            <table class="">
              <tbody>
                <tr>
                  <td class="cellTable">Tên nhóm</td>
                  <td class="cellTable">{{ GroupDetail.title }}</td>
                  <td class="cellTable">Quản trị hệ thống</td>
                  <td class="cellTable">
                    {{ formatAdministration(GroupDetail.is_admin) }}
                  </td>
                </tr>
                <!-- <tr>
                  <td class="cellTable">Người tạo</td>
                  <td class="cellTable">{{ GroupDetail.created_by_id }}</td>
  ...
  <teleport to="#app">
    ...
    <modal-delete
      ...
      :ConfirmDelete="handleToDeleteGroup"
    >
    </modal-delete>
    <!-- End Xác nhận xóa -->
  </teleport>
...
export default {
  ...
  setup() {
    ...
    const formatAdministration = (stt) => {
      if (stt == "yes") {
        return "Có";
      } else {
        return "Không";
    ...
    const handleToEditGroup = () => {
      router.push(`/admin-group-edit/${route.params.idGroup}`);
    };
  ...
  methods: {
    ...
    updateRoleGroup() {
      ...
      this.$store.dispatch("adminGroups/UpdateAdminGroupAction", {
        data: listRoleGroup,
        router: this.$router,
        toast: this.toast,
      });
