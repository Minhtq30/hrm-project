<template>
  <ul class="flex text-gray-400 py-3">
    <li class="px-4 py-1" :class="{ activeRouter: checkRoute('ALL') }">
      <router-link
        :to="
          '/approval-shiftresgister-view/type=' +
    ...
    <li
      class="px-4 py-1"
      :class="{
        activeRouter: checkRoute('WAITING'),
      }"
    >
      <router-link
        :to="
    ...
    <li
      class="px-4 py-1"
      :class="{
        activeRouter: checkRoute('APPROVING'),
      }"
    >
      <router-link
        :to="
    ...
    <li
      class="px-4 py-1"
      :class="{
        activeRouter: checkRoute('APPROVED'),
      }"
    >
      <router-link
        :to="
...
export default {
  setup() {
    ...
    const { CountTab } = storeToRefs(shiftresgister);
