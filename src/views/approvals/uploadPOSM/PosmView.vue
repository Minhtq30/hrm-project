  ...
  <person-layout>
    ...
    <Transition duration="550" name="slide-fade">
      <div class="w-full" v-if="isLoading == false">
        <div v-if="listUploadPosm?.length">
          <table-responsive
            :header="header"
    ...
    <template v-slot:sub_footer v-if="listUploadPosm?.length">
...
export default {
  ...
  setup() {
    ...
    const { listUploadPosm, listPagination } = storeToRefs(leave);
    ...
    watch(route, (to) => {
      ...
      if (
        to.path ==
        `/upload-posm-view/type=${to.params.type}&menu=${to.params.menu}`
      ) {
        leave.GetAllUploadPosmAction({
          type: to.params.type,
          menu: to.params.menu,
    ...
    const checkedAllItem = () => {
      if (!checkedAll.value) {
        ...
        for (var item in listUploadPosm.value) {
          checkLeave.value.push(listUploadPosm.value[item].ID);
        }
      } else {
    ...
    const updateApprovalLeave = () => {
      router.push(`/approval-posm-edit/${leaveDetail.value.ID}`);
    };
    const ShowDetail = (id) => {
      router.push(`/approval-posm-view/detail=${id}`);
    };
