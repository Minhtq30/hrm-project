      ...
      <navbar-social :isShowSearch="false"
        ><div class="flex grow">
          <div class="flex items-center">
            ...
            <div class="flex items-center">
              <p class="longText">Đơn check in/out</p>
            </div>
          </div>
        </div></navbar-social
      >
              ...
              <div class="w-full">
                <table class="w-full">
                  <thead class="bg-slate-50">
                    <tr>
                      <td class="cellTable font-medium">Ngày check in/out</td>
                      <td class="cellTable font-medium">Thời gian</td>
                      <td class="cellTable font-medium">Lý do</td>
                      <td class="cellTable font-medium">Tinh công</td>
                    </tr>
                  </thead>
                  <tbody>
                    <tr
                      ...
                      :key="index"
                    >
                      <td class="cellTable">{{ formatDate(item.date) }}</td>
                      <td class="cellTable">
                        {{ item.time }}
                      </td>
                      <td class="cellTable">{{ formatInout(item.reason) }}</td>
                      <td class="cellTable">{{ item.has_attendance }}</td>
                    </tr>
                  </tbody>
                </table>
              </div>
...
export default {
  ...
  setup() {
    ...
    watch(route, (to) => {
      if (to.name == "Chi tiết đơn check in/out") {
