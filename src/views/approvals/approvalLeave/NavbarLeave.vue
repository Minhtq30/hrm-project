<template>
  <ul class="flex text-gray-400 py-3">
    <li class="px-4 py-1" :class="{ activeRouter: checkRoute('ALL') }">
      <router-link
        :to="
          '/approval-leave-view/type=' + this.$route.params.type + '&menu=ALL'
        "
    ...
    <li class="px-4 py-1" :class="{ activeRouter: checkRoute('NO_APPROVED') }">
      <router-link
        :to="
          '/approval-leave-view/type=' +
          this.$route.params.type +
...
export default {
  setup() {
    ...
    const { CountTab } = storeToRefs(leave);
