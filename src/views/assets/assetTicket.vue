<template>
  <person-layout>
        ...
        <div class="flex grow">
          <div class="flex items-center ml-4">
            <div class="icon-slider button-create relative group">
              ...
              >
                <ul>
                  <li class="sub-item" @click="this.$router.push('/asset-add')">
                    <span class="ml-4">Thêm mới tài sản</span>
                  </li>
                </ul>
              </div>
            </div>
    ...
    <div class="w-full">
      <table-responsive
        ...
        >
          ...
          <td class="p-3">
            {{ assetTicket?.user_assignment_name }}
            <p>
              {{ assetTicket?.user_assignment_department }} /
              {{ assetTicket?.user_assignment_position }}
            </p>
          </td>

          <td class="p-3">
            {{ assetTicket?.user_assign_name }}
            <p>
              {{ assetTicket?.user_assign_department }} /
              {{ assetTicket?.user_assign_position }}
            </p>
          </td>

          <td class="p-3">{{ assetTicket.description }}</td>
        </tr>
      </table-responsive>
      <!--
    ...
    <template v-slot:sub_footer v-if="listAssetTicket?.length">
...
// import { useListAllTicket } from "@/store/modules/asset/assetTicket";
...
export default {
  ...
  setup() {
    ...
    const status1 = ref("");
    const value1 = ref(0);
    const isLoading = ref(store.state.assetTicket.isLoading ?? false);
    // const listall = useListAllTicket();
    ...
    const assetTicketUpdate = () => {
      router.push(`/asset-assign-update/${assetTicketDetail.value.id}`);
    };
    ...
    const ticketUpdate = () => {
      const type = assetTicketDetail.value.type;
      if (type == "ASSIGN") {
        router.push(`/asset-assign-update/${assetTicketDetail.value.id}`);
      } else if (type == "REPAIR") {
        router.push(`/asset-repair-update/${assetTicketDetail.value.id}`);
      } else if (type == "LOST") {
        router.push(`/asset-lost-update/${assetTicketDetail.value.id}`);
      } else if (type == "RECALL") {
        router.push(`/asset-recall-update/${assetTicketDetail.value.id}`);
      } else if (type == "DESTROY") {
        router.push(`/asset-destroy-update/${assetTicketDetail.value.id}`);
      } else if (type == "LIQUIDATED") {
        router.push(`/asset-liquidated-update/${assetTicketDetail.value.id}`);
      } else if (type == "BROKEN") {
        router.push(`/asset-broken-update/${assetTicketDetail.value.id}`);
      } else if (type == "REUSED") {
        router.push(`/asset-reused-update/${assetTicketDetail.value.id}`);
      } else if (type == "TRANSFER") {
        router.push(`/asset-transfer-update/${assetTicketDetail.value.id}`);
      }
    };
    return {
