            ...
            <div class="input">
              ...
              <div class="my-4 text-left" v-if="isHidden">
                <span class="text-gray-400 font-style: italic px-4"
                  >Các trường đánh dấu <span class="text-red-600">*</span> là
                  các trường bắt buộc</span
                >
              </div>
            </div>
            <div class="info">
...
export default {
  ...
  methods: {
    ...
    checkForm() {
      if (this.asset?.ticket != null) {
        const day1 = new Date(this.assetTicket.date_assign);
        const day2 = new Date(this.asset?.ticket.date_assign);
        if (day1 < day2) {
          this.toast.error("Ngày hỏng không được nhỏ hơn");
        } else {
          this.createAssetAssign();
        }
      } else {
        this.createAssetAssign();
      }
