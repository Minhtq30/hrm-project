<template>
  <person-layout>
      ...
      <navbar-social>
        <div class="flex grow">
          <div class="flex items-center">
            ...
            <div class="longText">
              Danh sách giảm dự kiến tháng {{ this.$route.params.month }}/{{
    ...
    <div class="w-full" v-if="isLoading == false">
      <div v-if="listInsuranceReduced?.length">
        <table-responsive
          :header="header"
        ...
        >
          <tr
            ...
            v-for="insurance in listInsuranceReduced"
    ...
    <template v-slot:sub_footer v-if="listInsuranceReduced?.length"
...
export default {
  ...
  setup() {
    ...
    const { listInsuranceReduced, listPagination } = storeToRefs(insurance);
    dataDate.value = {
    ...
    insurance.GetAllInsuranceReducedAction({
      data: dataDate.value,
      EndTimeLoading,
    });
    watch(route, (to) => {
      if (to.name == "Bảo hiểm dự kiến giảm") {
    ...
    const checkedAllItem = () => {
      if (!checkedAll.value) {
        ...
        for (var item in listInsuranceReduced.value) {
          checkInsurances.value.push(
            listInsuranceReduced.value[item].personnel_id
          );
        }
      } else {
    ...
    const ChangeDate = (event) => {
      ...
      router.push(
        `/personnel-insurance-reduced/month=${
          date.getMonth() + 1
